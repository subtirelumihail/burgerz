...
import type { CreateRestaurantReviewRequest } from "@/types/review";
...
function parseCreateReviewBody(
  body: unknown,
): CreateRestaurantReviewRequest | null {
  ...
  const image =
    record.image && typeof record.image === "object"
      ? (record.image as CreateRestaurantReviewRequest["image"])
      : undefined;
...
export async function POST(request: Request, { params }: ReviewsRouteParams) {
  ...
  const review = createRestaurantReview(id, payload);
